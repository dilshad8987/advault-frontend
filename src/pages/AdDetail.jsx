  ...
  return (
    <div style={{minHeight:'100vh',background:'#08080f',color:'#f0f0f8',fontFamily:'system-ui,sans-serif'}}>
      ...
      <div style={RP.section}>
        <div style={RP.inner}>
          <div style={RP.header}>
            <div style={RP.titleRow}>
              <span style={RP.sparkle}>✨</span>
              <h2 style={RP.title}>Related Products</h2>
              {!relatedLoading && relatedAds.length > 0 && (
                <span style={RP.badge}>{relatedAds.length} similar ads</span>
              )}
            </div>
            <p style={RP.subtitle}>Same industry ke trending ads — isse apne competitors ka analysis karo</p>
          </div>

          {relatedLoading ? (
            <div style={RP.loadingRow}>
              <div style={S.spinner}></div>
            ...
            <div style={RP.grid}>
              {relatedAds.map((a, i) => (
                <div
                  key={a.id||a.ad_id||a.material_id||i}
                ...
                >
                  <div style={RP.cardMedia}>
                    {a.video_info?.cover || a.imageUrl ? (
                      <img
                        src={a.video_info?.cover || a.imageUrl}
                        alt={a.ad_title || a.title || 'Ad'}
                        style={RP.cardImg}
                    ...
                    {(a.video_info || a.isVideo) && (
                      <span style={RP.videoBadge}>▶ Video</span>
                    )}
                    {(!a.last_shown_date || new Date(a.last_shown_date*1000) > new Date()) && (
                      <span style={RP.activeBadge}>● Live</span>
                    )}
                  </div>
                  <div style={RP.cardInfo}>
                    <div style={RP.cardBrand}>
                      {a.brand_name || a.advertiser_name || 'Unknown Brand'}
                    </div>
                    <div style={RP.cardTitle2}>
                      {(a.ad_title || a.title || 'Untitled').substring(0, 60)}{(a.ad_title||a.title||''). length > 60 ? '…' : ''}
                    </div>
