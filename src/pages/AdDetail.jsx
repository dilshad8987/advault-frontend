      ...
      <div style={S.page}>
        ...
        {activeTab==='overview' && (
          <div style={{display:'flex',flexDirection:'column',gap:'1.25rem',animation:'fadeIn .3s ease'}}>
            ...
            {/* RELATED PRODUCTS — Overview ke andar */}
            <div style={{background:'#0d0d1a',border:'1px solid rgba(108,71,255,.15)',borderRadius:'16px',padding:'1.5rem'}}>
              <div style={{display:'flex',alignItems:'center',gap:'.6rem',marginBottom:'.3rem',flexWrap:'wrap'}}>
                <span style={{fontSize:'1.2rem'}}>✨</span>
                <h3 style={{fontSize:'.95rem',fontWeight:700,color:'#f0f0f8',margin:0,flex:1}}>Related Products</h3>
                {!relatedLoading && relatedAds.length > 0 && (
                  <span style={{background:'linear-gradient(135deg,rgba(108,71,255,.2),rgba(139,107,255,.15))',border:'1px solid rgba(108,71,255,.35)',color:'#8b6bff',borderRadius:'20px',padding:'.2rem .8rem',fontSize:'.73rem',fontWeight:700}}>{relatedAds.length} similar ads</span>
                )}
              </div>
              <p style={{color:'#8888aa',fontSize:'.8rem',margin:'0 0 1.25rem',lineHeight:1.5}}>Same industry ke trending ads — competitors ka analysis karo</p>
              {relatedLoading ? (
                ...
                <div style={S.empty}>
                  ...
                  <p style={{color:'#8888aa',margin:0,fontSize:'.85rem'}}>Koi related ads nahi mile</p>
                </div>
              )}
            </div>
          </div>
