...
function VideoPlayer({ videoUrl, tiktokItemUrl, cover, title, adId, isMeta, fbSnapUrl }) {
  ...
  useEffect(() => {
    // Meta ads ke liye — R2 URL seedha use karo, API call mat karo
    if (isMeta) {
      if (videoUrl) {
  ...
  if (!videoUrl || videoError) return (
    <div style={VP.wrap}>
      ...
      {/* Meta ad — Facebook pe dekho button */}
      {isMeta && fbSnapUrl && (
        <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'rgba(8,8,15,0.75)',gap:'0.75rem'}}>
          {cover && <img src={cover} alt={title} style={{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'cover',zIndex:0,opacity:0.3}} />}
          <div style={{position:'relative',zIndex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:'0.75rem'}}>
            <a href={fbSnapUrl} target="_blank" rel="noreferrer"
              style={{background:'linear-gradient(135deg,#1877F2,#E1306C)',color:'#fff',padding:'.65rem 1.5rem',borderRadius:'10px',fontSize:'.85rem',fontWeight:700,textDecoration:'none',display:'flex',alignItems:'center',gap:'0.5rem'}}>
              📺 Facebook pe Video Dekho
            </a>
            <span style={{color:'rgba(255,255,255,.5)',fontSize:'.72rem'}}>New tab mein khulega</span>
          </div>
        </div>
      )}
      {/* TikTok error fallback */}
...
export default function AdDetail() {
  ...
  // ── View tracking — detail page khulne pe ──
  useEffect(() => {
  ...
  // ── Video URL ──
  // Meta: r2_video_url (permanent) > video (CDN, may expire) > snapshot
  // TikTok: video_info.video_url object
  ...
  const fbSnapUrl = isMeta ? (ad.snapshot_url || ad._raw?.snapshot_url || (ad.library_id ? 'https://www.facebook.com/ads/library/?id=' + ad.library_id : '')) : '';
      ...
      <div style={S.page}>
        ...
        {activeTab==='related' && (
          <div style={{animation:'fadeIn .3s ease'}}>
            ...
            <p style={{color:'#8888aa',fontSize:'.83rem',margin:'0 0 1.5rem',lineHeight:1.5}}>Same industry ke trending ads — competitors ka analysis karo</p>
            {relatedLoading ? (
...
const RP = {
  section:     {background:'#04040c',borderTop:'1px solid rgba(108,71,255,.12)',padding:'2.5rem clamp(1rem,4vw,2rem) 5rem'},
  inner:       {maxWidth:'1100px',margin:'0 auto'},
  header:      {marginBottom:'1.75rem'},
  titleRow:    {display:'flex',alignItems:'center',gap:'.65rem',flexWrap:'wrap',marginBottom:'.4rem'},
  sparkle:     {fontSize:'1.4rem'},
  title:       {fontSize:'clamp(1.1rem,3vw,1.4rem)',fontWeight:900,color:'#f0f0f8',margin:0,letterSpacing:'-.01em'},
  badge:       {background:'linear-gradient(135deg,rgba(108,71,255,.2),rgba(139,107,255,.15))',border:'1px solid rgba(108,71,255,.35)',color:'#8b6bff',borderRadius:'20px',padding:'.2rem .8rem',fontSize:'.75rem',fontWeight:700},
  subtitle:    {color:'#8888aa',fontSize:'.83rem',margin:0,lineHeight:1.5},
  loadingRow:  {display:'flex',alignItems:'center',gap:'1rem',padding:'2rem 0'},
