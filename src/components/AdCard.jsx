...
function proxyImageUrl(imageUrl, libraryId) {
  ...
  // Backend proxy se serve karo — kabhi expire nahi hogi
...
export default function AdCard({ ad, platform = 'tiktok' }) {
  ...
  // ── Image — proxy se serve karo (kabhi expire nahi hogi) ──
  const adLibraryId = isMeta ? adId : null;
  const rawCover  = isMeta
    ...
    : ttCover;
  // Meta ads ke liye proxy use karo — TikTok ke liye direct URL
  const cover = isMeta
    ? proxyImageUrl(fixImageUrl(rawCover), adLibraryId)
    : fixImageUrl(rawCover);
            ...
            <>
              ...
              <img
                ...
                onError={(e) => {
                  ...
                  const fb = e.target.parentElement.querySelector('.fb-snap');
              ...
              {fbSnapUrl && (
                <div
                  className="fb-snap"
                  style={{ display: 'none', position: 'absolute', inset: 0, zIndex: 2, flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(10,10,20,0.92)', gap: '0.75rem', cursor: 'pointer' }}
                  onClick={() => window.open(fbSnapUrl, '_blank')}
                >
                  <span style={{ fontSize: '2rem' }}>📺</span>
                  <span style={{ color: '#5aabff', fontSize: '0.82rem', fontWeight: 700 }}>View on Facebook</span>
                </div>
              )}
            </>
          ) : isMeta ? (
