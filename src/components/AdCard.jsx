...
function FbModal({ url, adData, onClose }) {
  const [iframeBlocked, setIframeBlocked] = React.useState(false);

  // Facebook iframe block karta hai X-Frame-Options se
  // Isliye 3 sec baad check karte hain
  React.useEffect(() => {
    ...
    <div style={s.modalOverlay} onClick={onClose}>
      <div style={s.modalBox} onClick={e => e.stopPropagation()}>
        ...
        <div style={s.modalBody}>
          {iframeBlocked ? (
            // Facebook iframe block karta hai — redirect dikhao
            <div style={s.modalFallback}>
              ...
              <p style={s.modalFallbackText}>
                Facebook security ki wajah se<br/>
                preview yahan nahi chal sakta
              </p>
              <a
                href={url}
                target="_blank"
              ...
              <p style={s.modalFallbackSub}>
                New tab mein khulega — video wahan play hogi
              </p>
            </div>
          ) : (
...
export default function AdCard({ ad, platform = 'tiktok' }) {
  ...
  return (
    <>
      {showModal && <FbModal url={fbSnapUrl} adData={modalAdData} onClose={() => setShowModal(false)} />}
