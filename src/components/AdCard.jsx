...
export default function AdCard({ ad, platform = 'tiktok' }) {
  ...
  const isVideoAd   = isMeta ? (ad.ad_type === 'video' || !!rawVideoUrl) : false;
  ...
  const rawVideoUrl = isMeta
    ? (
        ad.r2_video_url      ||   // R2 permanent URL (best)
        ad._raw?.r2_video_url ||
        ad.video_url         ||   // CDN URL (temporary)
        ad._raw?.video       ||
        ad.video             ||
        ''
      )
        ...
        <div style={s.media}>
          {thumbUrl && !thumbError ? (
            <>
              ...
              />
              {/* Video ad — play button dikhao */}
              {isVideoAd && rawVideoUrl && (
                <div style={s.playOverlay} onClick={openModal}>
                  <div style={s.playBtn}>▶</div>
                </div>
              )}
              {/* Video badge */}
              {isVideoAd && (
                <div style={s.vidBadge}>🎬 Video</div>
              )}
            </>
          ) : isMeta ? (
